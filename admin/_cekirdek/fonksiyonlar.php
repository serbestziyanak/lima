...
SQL;
	...
	*/
	...
	public function modulYetkileriVer( $id, $anahtarDegerAyni = false ) {
        ...
        if( !isset( $_SESSION["modul_yetkileri"] ) ){
            $sonuc					= $this->vt->select( self::SQL_yetki,  array( $id, $_SESSION[ "kullanici_turu" ] ) );
            $_SESSION["modul_yetkileri"] = $sonuc;
        }
        $sonuc = $_SESSION["modul_yetkileri"];
...
//////////////////////
	...
	*/
	...
	public function fn_resize($file, $target_file, $boyut) {
        $source_properties = getimagesize($file);
        $width = $source_properties[0];
        $height = $source_properties[1];
        ...
        if ($image_type == IMAGETYPE_JPEG) {
            ...
            imagejpeg($target_layer, $target_file);
        } elseif ($image_type == IMAGETYPE_GIF) {
            $image_resource_id = imagecreatefromgif($file);
            $target_layer = imagecreatetruecolor($target_width, $target_height);
            imagecopyresampled($target_layer, $image_resource_id, 0, 0, 0, 0, $target_width, $target_height, $width, $height);
            imagegif($target_layer, $target_file);
        } elseif ($image_type == IMAGETYPE_PNG) {
            $image_resource_id = imagecreatefrompng($file);
            $target_layer = imagecreatetruecolor($target_width, $target_height);
            imagecopyresampled($target_layer, $image_resource_id, 0, 0, 0, 0, $target_width, $target_height, $width, $height);
            imagepng($target_layer, $target_file);
        }
	}
