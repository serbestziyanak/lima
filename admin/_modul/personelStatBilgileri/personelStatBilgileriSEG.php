...
$SQL_tek_personel_stat_bilgileri_oku = <<< SQL
SELECT 
	*
FROM 
	tb_personel_statlar 
...
switch( $islem ) {
	case 'ekle':
		...
		$sorgu_sonuc = $vt->insert( $SQL_ekle, array(
			...
			,$_REQUEST[	'istihdam_asil_is_yeri_id' ]
			,$_REQUEST[	'stat_sayi_id' ]
			,$_REQUEST[	'buyruk_id' ]
			,$baslama_tarihi
			,$bitis_tarihi
			,$belge
	...
	break;
	case 'guncelle':
        $tek_personel_stat_bilgileri_oku = $vt->selectSingle( $SQL_tek_personel_stat_bilgileri_oku, array( $id ) ) [ 2 ];
		if( isset($_FILES["belge"]) and $_FILES["belge"]['size']>0 ){
			$belge = uniqid().basename($_FILES["belge"]["name"]);
			$target_dir = "../../belgeler/";
            $target_file = $target_dir . $belge;
			...
			$belge = $tek_personel_stat_bilgileri_oku['belge'];
		}
...
header( "Location:../../index.php?modul=personelStatBilgileri&personel_id=".$personel_id );
?>