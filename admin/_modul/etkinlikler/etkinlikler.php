		...
		<div class="row">
			...
			<div class="col-md-4">
				<div class="card card-secondary" id = "card_etkinlikler">
					<div class="card-header">
						<h3 class="card-title"><?php echo dil_cevir( "Etkinlikler", $dizi_dil, $sistem_dil ); ?></h3>
						...
						<table id="tbl_etkinlikler" class="table table-bordered table-hover table-sm" width = "100%" >
							...
							<tbody>
								...
								<tr oncontextmenu="fun();" class ="etkinlik-Tr <?php if( $etkinlik[ 'id' ] == $id ) echo $satir_renk; ?>" data-id="<?php echo $etkinlik[ 'id' ]; ?>">
									<td><?php echo $sayi++; ?></td>
									<td><?php echo $etkinlik[ 'dil_adi' ]; ?></td>
									...
									<td align = "center">
										<button modul= 'etkinlikler' yetki_islem="sil" class="btn btn-xs btn-danger" data-href="_modul/etkinlikler/etkinliklerSEG.php?islem=sil&id=<?php echo $etkinlik[ 'id' ]; ?>&birim_id=<?php echo $birim_id; ?>&birim_adi=<?php echo $birim_adi; ?>" data-toggle="modal" data-target="#sil_onay"><?php echo dil_cevir( "Sil", $dizi_dil, $sistem_dil ); ?></button>
									</td>
								</tr>
								<?php } ?>
							</tbody>
				...
				<div class="card <?php if( $id == 0 ) echo 'card-secondary' ?>">
					<div class="card-header p-2">
						<ul class="nav nav-pills tab-container">
							<?php if( $id > 0 ) { ?>
								<h6 style = 'font-size: 1rem;'> &nbsp;&nbsp;&nbsp; <?php echo dil_cevir( "Etkinlik Düzenle", $dizi_dil, $sistem_dil ); ?></h6>
							<?php } else { ?>
								<h6 style = 'font-size: 1rem;'> &nbsp;&nbsp;&nbsp; <?php echo dil_cevir( "Etkinlik Ekle", $dizi_dil, $sistem_dil ); ?></h6>
							<?php	}
							?>
						...
						<div class="tab-content">
							...
							<div class="tab-pane active" id="_genel">
								<form class="form-horizontal" action = "_modul/etkinlikler/etkinliklerSEG.php" method = "POST" enctype="multipart/form-data">
									...
									<input type="hidden"  name="<?php echo $anahtar;  ?>" value="<?php echo htmlentities($tek_etkinlik[$anahtar]);  ?>">
									<?php } ?>
									<div class="form-group">
										...
										<select class="form-control select2" name = "dil_id" required>
											...
											<option value="<?php echo $result["id"] ?>" <?php if( $tek_etkinlik['dil_id'] == $result["id"] ) echo "selected"; ?> ><?php echo $result["adi".$dil] ?></option>
											<?php } ?>
										</select>
									</div>
									...
									<div class="form-group">
										...
										<img src="resimler/etkinlikler/kucuk/<?php echo $tek_etkinlik[ 'foto' ]; ?>" width="200">
									</div>
									<?php } ?>
									<div class="form-group">
										...
										<div class="input-group date" id="tarih" data-target-input="nearest">
											...
											<input required type="text" data-target="#tarih" data-toggle="datetimepicker" name="tarih" value="<?php if( $tek_etkinlik[ 'tarih' ] !='' ){echo date('d.m.Y H:i',strtotime($tek_etkinlik[ 'tarih' ] ));}//else{ echo date('d.m.Y'); } ?>" class="form-control form-control-sm datetimepicker-input" autocomplete="off" />
										</div>
									</div>
									<div class="form-group">
									...
									<div class="form-group">
										<label class="control-label"><?php echo dil_cevir( "Etkinlik Yeri", $dizi_dil, $sistem_dil ); ?></label>
										<input required type="text" class="form-control" id ="yeri" name ="yeri" value = "<?php echo $tek_etkinlik[ "yeri" ]; ?>"  autocomplete="off">
									</div>
									<div class="form-group">
									...
									<div class="card-footer">
										<?php if( $birim_id >0 ){ ?>
										<button modul= 'etkinlikler' yetki_islem="kaydet" type="submit" class="<?php echo $kaydet_buton_cls; ?>"><span class="fa fa-save"></span> <?php echo dil_cevir( $kaydet_buton_yazi, $dizi_dil, $sistem_dil ); ?></button>
										<?php } ?>
									</div>
								</form>
							</div>
	...
	<script>
		...
		function dil_degistir(eleman){
			...
			<?php if( $islem == "guncelle" ){ ?>
				...
				document.getElementById("yeri").value = document.getElementsByName("yeri"+dil)[0].value;
