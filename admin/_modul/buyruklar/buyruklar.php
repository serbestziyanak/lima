...
if( array_key_exists( 'sonuclar', $_SESSION ) ) {
	$mesaj								= dil_cevir( $_SESSION[ 'sonuclar' ][ 'mesaj' ], $dizi_dil, $sistem_dil );
...
LEFT JOIN tb_personeller AS p ON p.id = s.personel_id
ORDER BY s.id DESC
SQL;

$SQL_tek_personel_buyruk_bilgi_oku = <<< SQL
SELECT 
	*
FROM 
...
@$tek_personel_buyruk_bilgi 	= $vt->selectSingle( $SQL_tek_personel_buyruk_bilgi_oku, array( $id ) )[ 2 ];
@$tek_personel                  = $vt->selectSingle( $SQL_personel, array( $personel_id ) )[ 2 ];

$ust_idler					= $vt->select( $SQL_ust_id_getir, array( $tek_personel_buyruk_bilgi['birim_id'] ) )[ 2 ];
$alt_idler					= $vt->select( $SQL_alt_id_getir, array( $tek_personel_buyruk_bilgi['birim_id'] ) )[ 2 ];
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-8">
                <div class="row">
                    <div class="col-12">
                        <div class="card card-olive" id = "card_personel_buyruk_bilgileri">
                            <div class="card-header">
                                <h3 class="card-title"><?php echo dil_cevir( "Buyruk Bilgileri", $dizi_dil, $sistem_dil ); ?></h3>
                                <div class = "card-tools">
                                    <button type="button" data-toggle = "tooltip" title = "Tam sayfa göster" class="btn btn-tool" data-card-widget="maximize"><i class="fas fa-expand fa-lg"></i></button>
                                    <a id = "yeni_fakulte" data-toggle = "tooltip" title = "Add" href = "?modul=buyruklar&islem=ekle&personel_id=<?php echo $personel_id ?>" class="btn btn-tool" ><i class="fas fa-plus fa-lg"></i></a>
                                </div>
                            </div>
                            <div class="card-body">
                                <table id="tbl_personel_buyruk_bilgileri" class="table table-bordered table-hover table-sm" width = "100%" >
                                    <thead>
                                        <tr>
                                            <th style="width: 15px">#</th>
                                            <th><?php echo dil_cevir( "Eğitim Öğretim Yılı", $dizi_dil, $sistem_dil ); ?></th>
                                            <th><?php echo dil_cevir( "Buyruk Türü", $dizi_dil, $sistem_dil ); ?></th>
                                            <th><?php echo dil_cevir( "Buyruk No", $dizi_dil, $sistem_dil ); ?></th>
                                            <th><?php echo dil_cevir( "Buyruk Tarihi", $dizi_dil, $sistem_dil ); ?></th>
                                            <th><?php echo dil_cevir( "Personeller", $dizi_dil, $sistem_dil ); ?></th>
                                    ...
                                    <tbody>
                                        <?php 
                                        $sayi = 1; 
                                        foreach( $personel_buyruk_bilgileri AS $personel_buyruk_bilgi ) { 
                                        ?>
                                        <tr oncontextmenu="fun();" class =" <?php if( $personel_buyruk_bilgi[ 'id' ] == $id ) echo $satir_renk; ?>" data-id="<?php echo $personel_buyruk_bilgi[ 'id' ]; ?>">
                                            <td><?php echo $sayi++; ?></td>
                                            <td><?php echo $personel_buyruk_bilgi[ 'egitim_ogretim_yili' ]; ?></td>
                                            <td><?php echo $personel_buyruk_bilgi[ 'buyruk_turu'.$dil ]; ?></td>
                                            <td><?php echo $personel_buyruk_bilgi[ 'buyruk_no' ]; ?></td>
                                            <td><?php echo $fn->tarihVer($personel_buyruk_bilgi[ 'buyruk_tarihi' ]); ?></td>
                                            <td>
                                                <?php 
                                                $where = "WHERE id IN (".$personel_buyruk_bilgi[ 'personel_idler' ].")";
                                                $SQL_buyruk_personelleri = <<< SQL
                                                SELECT
                                                    id,adi,adi_kz,adi_en,adi_ru,soyadi,soyadi_kz,soyadi_en,soyadi_ru
                                                FROM 
                                                    tb_personeller
                                                $where
                                                ORDER BY adi$dil
                                                SQL;               
                                                $buyruk_personelleri = $vt->select($SQL_buyruk_personelleri, array(  ) )[ 2 ];
                                                foreach( $buyruk_personelleri as $buyruk_personel ){
                                                    echo "<a href='?modul=personelDetay&personel_id=$buyruk_personel[id]' class='btn btn-xs bg-olive m-1' target='_blank' >".$buyruk_personel['adi'.$dil]." ".$buyruk_personel['soyadi'.$dil]."</a>";
                                                }
                                                ?>
                                            </td>
                                            <td>
                                                <?php if( strlen($personel_buyruk_bilgi[ 'belge' ]) > 2 ){ ?>
                                                <a href="<?php echo "belgeler/".$personel_buyruk_bilgi[ 'belge' ]; ?>" target="_blank" download><?php echo dil_cevir( "İndir", $dizi_dil, $sistem_dil ); ?></a>
                                                <?php } ?>
                                            </td>
                                            <td align = "center">
                                                <a modul = 'buyruklar' yetki_islem="duzenle" class = "btn btn-sm btn-warning btn-xs" href = "?modul=buyruklar&islem=guncelle&id=<?php echo $personel_buyruk_bilgi[ 'id' ]; ?>" >
                                                    <?php echo dil_cevir( "Düzenle", $dizi_dil, $sistem_dil ); ?>
                                                </a>
                                            </td>
                                            <td align = "center">
                                                <button modul= 'buyruklar' yetki_islem="sil" class="btn btn-xs btn-danger" data-href="_modul/buyruklar/buyruklarSEG.php?islem=sil&personel_id=<?php echo $personel_buyruk_bilgi[ 'personel_id' ]; ?>&id=<?php echo $personel_buyruk_bilgi[ 'id' ]; ?>" data-toggle="modal" data-target="#sil_onay"><?php echo dil_cevir( "Sil", $dizi_dil, $sistem_dil ); ?></button>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
			...
			<div class="col-md-4">
				<form class="form-horizontal" action = "_modul/buyruklar/buyruklarSEG.php" method = "POST" enctype="multipart/form-data">
					<div class="card card-secondary">
						<div class="card-header">
							<h3 class='card-title'><?php echo dil_cevir( "Buyruk Ekle / Düzenle", $dizi_dil, $sistem_dil ); ?></h3>
						</div>
						<div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-control-sm select2" name = "egitim_ogretim_yili_id" required  >
                                        ...
                                        <?php foreach( $egitim_ogretim_yillari AS $egitim_ogretim_yili ){ ?>
                                            <option value="<?php echo $egitim_ogretim_yili[ "id" ]; ?>" <?php echo $tek_personel_buyruk_bilgi[ "egitim_ogretim_yili_id" ] == $egitim_ogretim_yili[ "id" ] ? "selected" : null ?>><?php echo $egitim_ogretim_yili[ "adi" ]; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-control-sm select2" name = "buyruk_turu_id" required >
                                        ...
                                        <?php foreach( $buyruk_turleri AS $result ){ ?>
                                            <option value="<?php echo $result[ "id" ]; ?>" <?php echo $tek_personel_buyruk_bilgi[ "buyruk_turu_id" ] == $result[ "id" ] ? "selected" : null ?>><?php echo $result[ "adi".$dil ]; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="control-label"><?php echo dil_cevir( "Buyruk No", $dizi_dil, $sistem_dil ); ?></label>
                                    <input  required type="text" placeholder="Buyruk No" class="form-control form-control-sm" name ="buyruk_no" value = "<?php echo $tek_personel_buyruk_bilgi[ "buyruk_no" ]; ?>"  autocomplete="off">
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="input-group date" id="buyruk_tarihi" data-target-input="nearest">
                                        ...
                                        <input  required type="text" data-target="#buyruk_tarihi" data-toggle="datetimepicker" name="buyruk_tarihi" value="<?php if( $tek_personel_buyruk_bilgi[ 'buyruk_tarihi' ] !='' ){echo date('d.m.Y',strtotime($tek_personel_buyruk_bilgi[ 'buyruk_tarihi' ] ));}//else{ echo date('d.m.Y'); } ?>" class="form-control form-control-sm datetimepicker-input" data-target="#datetimepicker1"/>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select   class="form-control select2"  multiple="multiple" name = "personel_idler[]" data-close-on-select="false">
                                        ...
                                        ?>
                                            <option value = "<?php echo $result[ 'id' ]; ?>" <?php if( in_array($result[ 'id' ], $personeller2) ) echo 'selected'?>><?php echo $result[ 'adi' ]." ".$result[ 'soyadi' ]?>(<?php echo $result[ 'adi_kz' ]." ".$result[ 'soyadi_kz' ]?>)</option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group sabit card p-3">
                                    ...
                                    <br>
                                    <input <?php if( $islem == "ekle" ) echo "required" ?> type="file" class="" id ="belge" name ="belge"   >
                                </div>
						</div>
						<div class="card-footer">
							<button modul= 'buyruklar' yetki_islem="kaydet" type="submit" class="<?php echo $kaydet_buton_cls; ?>"><span class="fa fa-save"></span> <?php echo dil_cevir( $kaydet_buton_yazi, $dizi_dil, $sistem_dil ); ?></button>
						</div>
					</div>
				</form>
			</div>
...
var tbl_personel_buyruk_bilgileri = $( "#tbl_personel_buyruk_bilgileri" ).DataTable( {
...
} ).buttons().container().appendTo('#tbl_personel_buyruk_bilgileri_wrapper .col-md-6:eq(0)');



$('#card_personel_buyruk_bilgileri').on('maximized.lte.cardwidget', function() {
	var tbl_personel_buyruk_bilgileri = $( "#tbl_personel_buyruk_bilgileri" ).DataTable();
	var column = tbl_personel_buyruk_bilgileri.column(  tbl_personel_buyruk_bilgileri.column.length - 1 );
	column.visible( ! column.visible() );
	var column = tbl_personel_buyruk_bilgileri.column(  tbl_personel_buyruk_bilgileri.column.length - 2 );
	column.visible( ! column.visible() );
});

$('#card_personel_buyruk_bilgileri').on('minimized.lte.cardwidget', function() {
	var tbl_personel_buyruk_bilgileri = $( "#tbl_personel_buyruk_bilgileri" ).DataTable();
	var column = tbl_personel_buyruk_bilgileri.column(  tbl_personel_buyruk_bilgileri.column.length - 1 );
	column.visible( ! column.visible() );
	var column = tbl_personel_buyruk_bilgileri.column(  tbl_personel_buyruk_bilgileri.column.length - 2 );
