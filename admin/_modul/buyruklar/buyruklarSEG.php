...
$personel_idler = implode(",", $_REQUEST[ 'personel_idler' ]);
...
$SQL_buyruk_no_varmi = <<< SQL
SELECT 
	count(*) as buyruk_sayisi
FROM 
	tb_personel_buyruklar 
WHERE 
	buyruk_no = ?
...
switch( $islem ) {
	case 'ekle':
        ...
        $buyruk_sayisi = $vt->selectSingle( $SQL_buyruk_no_varmi, array( $_REQUEST[	'buyruk_no' ] ) ) [ 2 ]['buyruk_sayisi'];
        if( $buyruk_sayisi > 0 ){
            $___islem_sonuc = array( 'hata' => true, 'mesaj' => 'Buyruk no zaten kayıtlı.', 'id' => $sorgu_sonuc[ 2 ] ); 
        }else{
...
if( $_REQUEST['gelen_yer'] == "buyruk_modal" ){
    ...
    $where = " WHERE personel_idler like '$_REQUEST[personel_id]' or personel_idler like '%,$_REQUEST[personel_id]' or personel_idler like '$_REQUEST[personel_id],%' or personel_idler like '%,$_REQUEST[personel_id],%'";
...
header( "Location:../../index.php?modul=buyruklar" );
?>