...
$SQL_tum_personel_vize_bilgileri = <<< SQL
SELECT
	 pvb.*
...
$SQL_tek_personel_vize_bilgi_oku = <<< SQL
SELECT 
	*
FROM 
	tb_personel_vize_bilgileri
WHERE 
	id = ?
SQL;
...
$vize_turleri 	            	= $vt->select($SQL_vize_turleri, array(  ) )[ 2 ];
...
$personel_vize_bilgileri	    = $vt->select( $SQL_tum_personel_vize_bilgileri, array( $personel_id ) )[ 2 ];
$ulkeler                   	    = $vt->select( $SQL_ulkeler, array(  ) )[ 2 ];
$egitim_ogretim_yillari         = $vt->select( $SQL_egitim_ogretim_yillari, array(  ) )[ 2 ];
@$tek_personel_vize_bilgi 	= $vt->selectSingle( $SQL_tek_personel_vize_bilgi_oku, array( $id ) )[ 2 ];
@$tek_personel                  = $vt->selectSingle( $SQL_personel, array( $personel_id ) )[ 2 ];

$ust_idler					= $vt->select( $SQL_ust_id_getir, array( $tek_personel_vize_bilgi['birim_id'] ) )[ 2 ];
$alt_idler					= $vt->select( $SQL_alt_id_getir, array( $tek_personel_vize_bilgi['birim_id'] ) )[ 2 ];
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-8">
                <div class="row">
                    <div class="col-12 ">
                        <div class="card card-danger card-outline" id = "card_personel_vize_bilgileri">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card card-olive" id = "card_personel_vize_bilgileri">
                            <div class="card-header">
                                <h3 class="card-title"><?php echo dil_cevir( "Vize Bilgiler", $dizi_dil, $sistem_dil ); ?></h3>
                                <div class = "card-tools">
                                    <button type="button" data-toggle = "tooltip" title = "Tam sayfa göster" class="btn btn-tool" data-card-widget="maximize"><i class="fas fa-expand fa-lg"></i></button>
                                    <a id = "yeni_fakulte" data-toggle = "tooltip" title = "Add" href = "?modul=personelVizeBilgileri&islem=ekle&personel_id=<?php echo $personel_id ?>" class="btn btn-tool" ><i class="fas fa-plus fa-lg"></i></a>
                                </div>
                            </div>
                            <div class="card-body">
                                <table id="tbl_personel_vize_bilgileri" class="table table-bordered table-hover table-sm" width = "100%" >
                                    <thead>
                                        <tr>
                                            <th style="width: 15px">#</th>
                                            <th><?php echo dil_cevir( "Eğitim Öğretim Yılı", $dizi_dil, $sistem_dil ); ?></th>
                                            <th><?php echo dil_cevir( "Belgeyi Veren Kurum", $dizi_dil, $sistem_dil ); ?></th>
                                    ...
                                    <tbody>
                                        <?php 
                                        $sayi = 1; 
                                        foreach( $personel_vize_bilgileri AS $personel_vize_bilgi ) { 
                                        ?>
                                        <tr oncontextmenu="fun();" class =" <?php if( $personel_vize_bilgi[ 'id' ] == $id ) echo $satir_renk; ?>" data-id="<?php echo $personel_vize_bilgi[ 'id' ]; ?>">
                                            <td><?php echo $sayi++; ?></td>
                                            <td><?php echo $personel_vize_bilgi[ 'egitim_ogretim_yili' ]; ?></td>
                                            <td><?php echo $personel_vize_bilgi[ 'belgeyi_veren_kurum'.$dil ]; ?></td>
                                            <td><?php echo $fn->tarihVer($personel_vize_bilgi[ 'baslama_tarihi' ]); ?></td>
                                            <td><?php echo $fn->tarihVer($personel_vize_bilgi[ 'bitis_tarihi' ]); ?></td>
                                            <td align = "center">
                                                <a modul = 'personelVizeBilgileri' yetki_islem="duzenle" class = "btn btn-sm btn-warning btn-xs" href = "?modul=personelVizeBilgileri&islem=guncelle&personel_id=<?php echo $personel_vize_bilgi[ 'personel_id' ]; ?>&id=<?php echo $personel_vize_bilgi[ 'id' ]; ?>" >
                                                    <?php echo dil_cevir( "Düzenle", $dizi_dil, $sistem_dil ); ?>
                                                </a>
                                            </td>
                                            <td align = "center">
                                                <button modul= 'personelVizeBilgileri' yetki_islem="sil" class="btn btn-xs btn-danger" data-href="_modul/personelVizeBilgileri/personelVizeBilgileriSEG.php?islem=sil&personel_id=<?php echo $personel_vize_bilgi[ 'personel_id' ]; ?>&id=<?php echo $personel_vize_bilgi[ 'id' ]; ?>" data-toggle="modal" data-target="#sil_onay"><?php echo dil_cevir( "Sil", $dizi_dil, $sistem_dil ); ?></button>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
			...
			<div class="col-md-4">
				<form class="form-horizontal" action = "_modul/personelVizeBilgileri/personelVizeBilgileriSEG.php" method = "POST" enctype="multipart/form-data">
					<div class="card card-secondary">
						<div class="card-header">
							<h3 class='card-title'><?php echo dil_cevir( "Vize Ekle / Düzenle", $dizi_dil, $sistem_dil ); ?></h3>
						</div>
						<div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-control-sm select2" name = "egitim_ogretim_yili_id" required  >
                                        ...
                                        <?php foreach( $egitim_ogretim_yillari AS $egitim_ogretim_yili ){ ?>
                                            <option value="<?php echo $egitim_ogretim_yili[ "id" ]; ?>" <?php echo $tek_personel_vize_bilgi[ "egitim_ogretim_yili_id" ] == $egitim_ogretim_yili[ "id" ] ? "selected" : null ?>><?php echo $egitim_ogretim_yili[ "adi" ]; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label  class="control-label"><?php echo dil_cevir( "Belgeyi Veren Kurum", $dizi_dil, $sistem_dil ); ?></label>
                                    <select class="form-control form-control-sm select2" name = "belgeyi_veren_kurum_id" required  >
                                        <option value=""><?php echo dil_cevir( "Seçiniz", $dizi_dil, $sistem_dil ); ?>...</option>
                                        <?php foreach( $belgeyi_veren_kurumlar AS $belgeyi_veren_kurum ){ ?>
                                            <option value="<?php echo $belgeyi_veren_kurum[ "id" ]; ?>" <?php echo $tek_personel_vize_bilgi[ "belgeyi_veren_kurum_id" ] == $belgeyi_veren_kurum[ "id" ] ? "selected" : null ?>><?php echo $belgeyi_veren_kurum[ "adi".$dil ]; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group date" id="bitis_tarihi" data-target-input="nearest">
                                        ...
                                        <input  type="text" data-target="#bitis_tarihi" data-toggle="datetimepicker" name="bitis_tarihi" value="<?php if( $tek_personel_vize_bilgi[ 'bitis_tarihi' ] !='' ){echo date('d.m.Y',strtotime($tek_personel_vize_bilgi[ 'bitis_tarihi' ] ));}//else{ echo date('d.m.Y'); } ?>" class="form-control form-control-sm datetimepicker-input" data-target="#datetimepicker1"/>
                                    </div>
                                </div>
						...
						<div class="card-footer">
							<button modul= 'personelVizeBilgileri' yetki_islem="kaydet" type="submit" class="<?php echo $kaydet_buton_cls; ?>"><span class="fa fa-save"></span> <?php echo dil_cevir( $kaydet_buton_yazi, $dizi_dil, $sistem_dil ); ?></button>
						</div>
					</div>
				</form>
			</div>
...
var tbl_personel_vize_bilgileri = $( "#tbl_personel_vize_bilgileri" ).DataTable( {
...
} ).buttons().container().appendTo('#tbl_personel_vize_bilgileri_wrapper .col-md-6:eq(0)');



$('#card_personel_vize_bilgileri').on('maximized.lte.cardwidget', function() {
	var tbl_personel_vize_bilgileri = $( "#tbl_personel_vize_bilgileri" ).DataTable();
	var column = tbl_personel_vize_bilgileri.column(  tbl_personel_vize_bilgileri.column.length - 1 );
	column.visible( ! column.visible() );
	var column = tbl_personel_vize_bilgileri.column(  tbl_personel_vize_bilgileri.column.length - 2 );
	column.visible( ! column.visible() );
});

$('#card_personel_vize_bilgileri').on('minimized.lte.cardwidget', function() {
	var tbl_personel_vize_bilgileri = $( "#tbl_personel_vize_bilgileri" ).DataTable();
	var column = tbl_personel_vize_bilgileri.column(  tbl_personel_vize_bilgileri.column.length - 1 );
	column.visible( ! column.visible() );
	var column = tbl_personel_vize_bilgileri.column(  tbl_personel_vize_bilgileri.column.length - 2 );
