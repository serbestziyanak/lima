...
$SQL_tum_personel_emekli_bilgileri = <<< SQL
SELECT 
	s.*
    ,eoy.adi    AS egitim_ogretim_yili
FROM 
	tb_personel_emekli_bilgileri as s
LEFT JOIN tb_egitim_ogretim_yillari AS eoy ON eoy.id = s.egitim_ogretim_yili_id
WHERE 
	s.personel_id = ?
ORDER BY s.emeklilik_tarihi DESC
SQL;

$SQL_tek_personel_emekli_bilgi_oku = <<< SQL
SELECT 
	*
FROM 
...
$emekli_turleri 	            	= $vt->select($SQL_emekli_turleri, array(  ) )[ 2 ];
...
$personel_emekli_bilgileri	    = $vt->select( $SQL_tum_personel_emekli_bilgileri, array( $personel_id ) )[ 2 ];
$ulkeler                   	    = $vt->select( $SQL_ulkeler, array(  ) )[ 2 ];
$egitim_ogretim_yillari         = $vt->select( $SQL_egitim_ogretim_yillari, array(  ) )[ 2 ];
@$tek_personel_emekli_bilgi 	= $vt->selectSingle( $SQL_tek_personel_emekli_bilgi_oku, array( $id ) )[ 2 ];
@$tek_personel                  = $vt->selectSingle( $SQL_personel, array( $personel_id ) )[ 2 ];

$ust_idler					= $vt->select( $SQL_ust_id_getir, array( $tek_personel_emekli_bilgi['birim_id'] ) )[ 2 ];
$alt_idler					= $vt->select( $SQL_alt_id_getir, array( $tek_personel_emekli_bilgi['birim_id'] ) )[ 2 ];
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-8">
                <div class="row">
                    <div class="col-12 ">
                        <div class="card card-danger card-outline" id = "card_personel_emekli_bilgileri">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card card-olive" id = "card_personel_emekli_bilgileri">
                            <div class="card-header">
                                <h3 class="card-title"><?php echo dil_cevir( "Emekli Bilgileri", $dizi_dil, $sistem_dil ); ?></h3>
                                <div class = "card-tools">
                                    <button type="button" data-toggle = "tooltip" title = "Tam sayfa göster" class="btn btn-tool" data-card-widget="maximize"><i class="fas fa-expand fa-lg"></i></button>
                                    <a id = "yeni_fakulte" data-toggle = "tooltip" title = "Add" href = "?modul=personelEmekliBilgileri&islem=ekle&personel_id=<?php echo $personel_id ?>" class="btn btn-tool" ><i class="fas fa-plus fa-lg"></i></a>
                                </div>
                            </div>
                            <div class="card-body">
                                <table id="tbl_personel_emekli_bilgileri" class="table table-bordered table-hover table-sm" width = "100%" >
                                    <thead>
                                        <tr>
                                            <th style="width: 15px">#</th>
                                            <th><?php echo dil_cevir( "Eğitim Öğretim Yılı", $dizi_dil, $sistem_dil ); ?></th>
                                            <th><?php echo dil_cevir( "Emeklilik Tarihi", $dizi_dil, $sistem_dil ); ?></th>
                                    ...
                                    <tbody>
                                        <?php 
                                        $sayi = 1; 
                                        foreach( $personel_emekli_bilgileri AS $personel_emekli_bilgi ) { 
                                        ?>
                                        <tr oncontextmenu="fun();" class =" <?php if( $personel_emekli_bilgi[ 'id' ] == $id ) echo $satir_renk; ?>" data-id="<?php echo $personel_emekli_bilgi[ 'id' ]; ?>">
                                            <td><?php echo $sayi++; ?></td>
                                            <td><?php echo $personel_emekli_bilgi[ 'egitim_ogretim_yili' ]; ?></td>
                                            <td><?php echo $fn->tarihVer($personel_emekli_bilgi[ 'emeklilik_tarihi' ]); ?></td>
                                            <td align = "center">
                                                <a modul = 'personelEmekliBilgileri' yetki_islem="duzenle" class = "btn btn-sm btn-warning btn-xs" href = "?modul=personelEmekliBilgileri&islem=guncelle&personel_id=<?php echo $personel_emekli_bilgi[ 'personel_id' ]; ?>&id=<?php echo $personel_emekli_bilgi[ 'id' ]; ?>" >
                                                    <?php echo dil_cevir( "Düzenle", $dizi_dil, $sistem_dil ); ?>
                                                </a>
                                            </td>
                                            <td align = "center">
                                                <button modul= 'personelEmekliBilgileri' yetki_islem="sil" class="btn btn-xs btn-danger" data-href="_modul/personelEmekliBilgileri/personelEmekliBilgileriSEG.php?islem=sil&personel_id=<?php echo $personel_emekli_bilgi[ 'personel_id' ]; ?>&id=<?php echo $personel_emekli_bilgi[ 'id' ]; ?>" data-toggle="modal" data-target="#sil_onay"><?php echo dil_cevir( "Sil", $dizi_dil, $sistem_dil ); ?></button>
                                            </td>
                                        </tr>
                                        <?php } ?>
                                    </tbody>
			...
			<div class="col-md-4">
				<form class="form-horizontal" action = "_modul/personelEmekliBilgileri/personelEmekliBilgileriSEG.php" method = "POST" enctype="multipart/form-data">
					<div class="card card-secondary">
						<div class="card-header">
							<h3 class='card-title'><?php echo dil_cevir( "Emeklilik Ekle / Düzenle", $dizi_dil, $sistem_dil ); ?></h3>
						</div>
						<div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-control-sm select2" name = "egitim_ogretim_yili_id" required  >
                                        ...
                                        <?php foreach( $egitim_ogretim_yillari AS $egitim_ogretim_yili ){ ?>
                                            <option value="<?php echo $egitim_ogretim_yili[ "id" ]; ?>" <?php echo $tek_personel_emekli_bilgi[ "egitim_ogretim_yili_id" ] == $egitim_ogretim_yili[ "id" ] ? "selected" : null ?>><?php echo $egitim_ogretim_yili[ "adi" ]; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label class="control-label"><?php echo dil_cevir( "Emeklilik Tarihi", $dizi_dil, $sistem_dil ); ?></label>
                                    <div class="input-group date" id="emeklilik_tarihi" data-target-input="nearest">
                                        <div class="input-group-append" data-target="#emeklilik_tarihi" data-toggle="datetimepicker">
                                            <div class="input-group-text"><i class="fa fa-calendar"></i></div>
                                        </div>
                                        <input  type="text" data-target="#emeklilik_tarihi" data-toggle="datetimepicker" name="emeklilik_tarihi" value="<?php if( $tek_personel_emekli_bilgi[ 'emeklilik_tarihi' ] !='' ){echo date('d.m.Y',strtotime($tek_personel_emekli_bilgi[ 'emeklilik_tarihi' ] ));}//else{ echo date('d.m.Y'); } ?>" class="form-control form-control-sm datetimepicker-input" data-target="#datetimepicker1"/>
                                    </div>
                                </div>
                                <div class="form-group">
                                        ...
                                        <div class="col-8">
                                            <select class="form-control form-control-sm select2" name = "buyruk_id" id="buyruk_id" >
                                                ...
                                                <?php foreach( $personel_buyruk_bilgileri AS $result ){ ?>
                                                    <option value="<?php echo $result[ "id" ]; ?>" <?php echo $tek_personel_emekli_bilgi[ "buyruk_id" ] == $result[ "id" ] ? "selected" : null ?>><?php echo $result[ "buyruk_no" ]." (".$result[ "buyruk_turu".$dil ].")"; ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                        <div class="col-4">
						...
						<div class="card-footer">
							<button modul= 'personelEmekliBilgileri' yetki_islem="kaydet" type="submit" class="<?php echo $kaydet_buton_cls; ?>"><span class="fa fa-save"></span> <?php echo dil_cevir( $kaydet_buton_yazi, $dizi_dil, $sistem_dil ); ?></button>
						</div>
					</div>
				</form>
			</div>
...
<script type="text/javascript">
	...
	$(function () {
		$('#emeklilik_tarihi').datetimepicker({
			//defaultDate: simdi,
			format: 'DD.MM.yyyy',
			icons: {
			time: "far fa-clock",
...
var tbl_personel_emekli_bilgileri = $( "#tbl_personel_emekli_bilgileri" ).DataTable( {
...
} ).buttons().container().appendTo('#tbl_personel_emekli_bilgileri_wrapper .col-md-6:eq(0)');



$('#card_personel_emekli_bilgileri').on('maximized.lte.cardwidget', function() {
	var tbl_personel_emekli_bilgileri = $( "#tbl_personel_emekli_bilgileri" ).DataTable();
	var column = tbl_personel_emekli_bilgileri.column(  tbl_personel_emekli_bilgileri.column.length - 1 );
	column.visible( ! column.visible() );
	var column = tbl_personel_emekli_bilgileri.column(  tbl_personel_emekli_bilgileri.column.length - 2 );
	column.visible( ! column.visible() );
});

$('#card_personel_emekli_bilgileri').on('minimized.lte.cardwidget', function() {
	var tbl_personel_emekli_bilgileri = $( "#tbl_personel_emekli_bilgileri" ).DataTable();
	var column = tbl_personel_emekli_bilgileri.column(  tbl_personel_emekli_bilgileri.column.length - 1 );
	column.visible( ! column.visible() );
	var column = tbl_personel_emekli_bilgileri.column(  tbl_personel_emekli_bilgileri.column.length - 2 );
