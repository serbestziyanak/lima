...
if( $_REQUEST[ 'odul_tarihi' ] == '' ) $odul_tarihi = NULL;
else $odul_tarihi = date( 'Y-m-d', strtotime( $_REQUEST[ 'odul_tarihi' ] ) );
...
$SQL_tek_personel_odul_bilgileri_oku = <<< SQL
SELECT 
	*
FROM 
	tb_personel_odul_bilgileri 
...
switch( $islem ) {
	case 'ekle':
		...
		$sorgu_sonuc = $vt->insert( $SQL_ekle, array(
			...
			,$_REQUEST[	'odulu_veren_kurum_id' ]
			,$_REQUEST[	'odul_turu_id' ]
			,$_REQUEST[	'odul_adi_id' ]
			,$odul_tarihi
			,$belge
		) );
	...
	break;
	case 'guncelle':
        $tek_personel_odul_bilgileri_oku = $vt->selectSingle( $SQL_tek_personel_odul_bilgileri_oku, array( $id ) ) [ 2 ];
		if( isset($_FILES["belge"]) and $_FILES["belge"]['size']>0 ){
			$belge = uniqid().basename($_FILES["belge"]["name"]);
			$target_dir = "../../belgeler/";
            $target_file = $target_dir . $belge;
			...
			$belge = $tek_personel_odul_bilgileri_oku['belge'];
		}
...
header( "Location:../../index.php?modul=personelOdulBilgileri&personel_id=".$personel_id );
?>