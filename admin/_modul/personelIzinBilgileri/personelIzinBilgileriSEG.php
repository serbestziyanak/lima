...
$SQL_tek_personel_izin_bilgileri_oku = <<< SQL
SELECT 
	*
FROM 
	tb_personel_izinler 
...
switch( $islem ) {
	...
	break;
	case 'guncelle':
        $tek_personel_izin_bilgileri_oku = $vt->selectSingle( $SQL_tek_personel_izin_bilgileri_oku, array( $id ) ) [ 2 ];
		if( isset($_FILES["belge"]) and $_FILES["belge"]['size']>0 ){
			$belge = uniqid().basename($_FILES["belge"]["name"]);
			$target_dir = "../../belgeler/";
            $target_file = $target_dir . $belge;
			...
			$belge = $tek_personel_izin_bilgileri_oku['belge'];
		}
...
header( "Location:../../index.php?modul=personelIzinBilgileri&personel_id=".$personel_id );
?>